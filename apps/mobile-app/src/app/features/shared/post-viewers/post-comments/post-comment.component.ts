...
export class PostCommentComponent implements OnInit, OnDestroy {
  ...
  onActionTakenClick() {
    setTimeout(() => {
      if (!this.comment.action_taken && !this.comment.no_action_taken) {
        return;
      }
  ...
  onApplyClick(data) {
    this.linkedPost = cloneDeep(data.post);
    if (data) {
      const isExist = find(this.comment.linked_reports, (o) => o.id === data.post.id);
      if (!isExist) {
        const obj = {
          id: data.post.id,
